...
// Simple PubSub system, optimized for observability and ease of use.
// Based on the NATS messaging system's DSL for subject subscription.
type PubSub struct {
...
// The callback code will be run synchronously (immediately) on the goroutine which calls Pub.
// The callback takes the subject as a first argument, and message as the second.
func sub(ps *PubSub, subject string, handler any, options ...SubOption) context.CancelFunc {
	...
	id := ulid.Make().String()
	sub := sublist.Subscription{Subject: []byte(subject), Value: handler, ID: id, Debug: opts.Debug}

	// Gather file and line information for subscription debugging purposes and,
	// if it's available, include this in the Subscription struct
...
// The callback code will be run synchronously on the goroutine which calls Pub.
// The callback takes the subject as a first argument, and message as the second.
func Sub[M any](ps *PubSub, subject string, handler func(string, M), options ...SubOption) context.CancelFunc {
	options = append(options, WithSkip(1)) // Skip this stack frame when recording the subscriber
	return sub(ps, subject, func(subject string, message any) {
		// The message might be nil, which we need to handle specially.
		// Or, later, we might mandate non-nil messages. But for now do this.
		if message == nil {
			var zero M
			handler(subject, zero)
		} else {
			handler(subject, message.(M))
		}
	}, options...)
}
...
func DebugSub(ps *PubSub, subject string, handler func(string, any, *sublist.SublistResult), options ...SubOption) context.CancelFunc {
	options = append(options, WithDebug(), WithSkip(1)) // Skip this stack frame when recording the subscriber
	return sub(ps, subject, handler, options...)
}

// Returns a channel on which messages are sent. The user is NOT responsible for closing the channel.
// Both the subscription and channel will be closed once the context completes.
func SubChan[M any](ps *PubSub, ctx context.Context, subject string, bufSize int, options ...SubOption) <-chan M {
	options = append(options, WithSkip(1)) // Skip this stack frame when recording the subscriber
	ch := make(chan M, bufSize)
	cancel := Sub(ps, subject, func(subject string, message M) {
		select {
		case ch <- message:
		case <-ctx.Done():
		}
...
func Pub[M any](ps *PubSub, subject string, message M) {
	...
	for _, sub := range matches.Psubs {
		if sub.Debug {
			...
			handler(subject, message, matches)
		} else {
	...
	// So we've seen that this works, but why would anyone think to do this in the first place?
...
// Used by the pubsub system to send publish nil messages.
// Each subscriber interprets the nil as the zero value of the subscriber's message type.
var NilAny any

// TODO: Batch pub. Would mean the whole batch is delivered to the first guy, then second, ...
